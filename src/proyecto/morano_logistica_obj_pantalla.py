...
Autor: Pablo Nahra
Objetivo:
Importar dos planillas Excel con los objetivos generales y objetivos por usuarios del trabajo
de logística
'''
import os
...
def ejecutar_proceso():
	...
	# Verificar existencia de archivos con extensión .xlsx
	ruta_generales = os.path.join(config_logist_obj.dir_planilla_objetivos,
	                              config_logist_obj.planilla_objetivos + ".xlsx")
	ruta_usuarios = os.path.join(config_logist_obj.dir_planilla_objetivos,
	                             config_logist_obj.planilla_objetivos_usuarios + ".xlsx")

	# Variable para controlar si se procesaron los generales
	proceso_generales_exitoso = False
	nro_proceso = None

	# =================================================================
	# PROCESO PARA OBJETIVOS GENERALES
	# =================================================================
	if os.path.exists(ruta_generales):
		...
		funciones_generales.log_grabar('Logistica - Objetivos - Generales - Inicio',
		...
		try:
			...
			nro_proceso = func_baseintermedia.obtener_nuevo_nro_proceso(
				...
				tabla=config_logist_obj.tabla_datosexcel
			)
			...
			objetivos = funciones_generales.leer_excel_y_convertir_a_lista(ruta_generales, titulo=0, datos=0)
			...
			funciones_generales.log_grabar('planillas_objetivos grales - inicio - ejecutar_proceso()',
			                               config_logist_obj.dir_log)
			...
			proceso_generales_exitoso = True
			ventana.mainloop()
		...
		funciones_generales.log_grabar('Logistica - Objetivos - Generales - Fin',
		                               config_logist_obj.dir_log)
	else:
		messagebox.showerror("Archivo no encontrado",
		                     f"El archivo {config_logist_obj.planilla_objetivos}.xlsx no se encuentra en el directorio:\n{config_logist_obj.dir_planilla_objetivos}\n\nNO puede ejecutarse el programa para objetivos generales.")

	# =================================================================
	# PROCESO PARA OBJETIVOS DE USUARIOS
	# =================================================================
	if os.path.exists(ruta_usuarios):
		...
		try:
			...
			# Si ya tenemos un nro_proceso de generales, lo usamos, si no creamos uno nuevo
			if nro_proceso is None:
				nro_proceso = func_baseintermedia.obtener_nuevo_nro_proceso(
					...
					sql_user=config_logist_obj.sql_user_int,
					sql_pass=config_logist_obj.sql_pass_int,
					tabla=config_logist_obj.tabla_objetivos_usuarios
				)
			actualizar_progreso(progreso, ventana, 30)

			# Leer el Excel
			funciones_generales.log_grabar('ruta_archivo - inicio - ejecutar_proceso()', config_logist_obj.dir_log)
			objetivos = funciones_generales.leer_excel_y_convertir_a_lista(ruta_usuarios, titulo=0, datos=0)
			actualizar_progreso(progreso, ventana, 50)

			# Insertar datos en tabla intermedia y tengo dos listas, las a integrar y la NO a integrar
			funciones_generales.log_grabar('planillas_objetivos_usuarios - inicio - ejecutar_proceso()',
			                               config_logist_obj.dir_log)

			# Listamos los usuarios existentes
			list_usuarios = func_baseintermedia.obtener_usuarios(
				sql_server=config_logist_obj.sql_server_int,
				sql_db=config_logist_obj.sql_db_int,
				sql_user=config_logist_obj.sql_user_int,
				sql_pass=config_logist_obj.sql_pass_int
			...
			lista_obj_filt, list_obj_no_integrar = func_baseintermedia.insert_datos_excel_usuarios(
				sql_server=config_logist_obj.sql_server_int,
				sql_db=config_logist_obj.sql_db_int,
				sql_user=config_logist_obj.sql_user_int,
				sql_pass=config_logist_obj.sql_pass_int,
		...
		messagebox.showerror("Archivo no encontrado",
		                     f"El archivo {config_logist_obj.planilla_objetivos_usuarios}.xlsx no se encuentra en el directorio:\n{config_logist_obj.dir_planilla_objetivos}\n\nNO puede ejecutarse el programa para objetivos de usuarios.")
